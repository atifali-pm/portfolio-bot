...
- [x] **Phase 0 DONE 2026-05-07.** Worker scaffolded with `src/index.ts`, `wrangler.toml` configured with `AI` + `VECTOR_INDEX` bindings, Vectorize index `portfolio-bot` created (768-dim cosine, matches `bge-base-en-v1.5`), `wrangler dev` boots in 1s with `/`, `/health`, and 404 fallback all responding correctly. Generated runtime types via `wrangler types` (replaces the now-deprecated `@cloudflare/workers-types`); added `@types/node` for `nodejs_compat` flag.
- [x] **Phase 1 DONE 2026-05-07.** Worker exposes `/admin/ingest` + `/admin/query` (Bearer-token auth via `INGEST_SECRET` secret). Local `scripts/ingest.ts` reads `../portfolio/website/src/data/projects.ts`, builds Markdown text per project, POSTs to the Worker which chunks (`src/chunker.ts`, ~500 char target with 100 char overlap), embeds via `@cf/baai/bge-base-en-v1.5`, and upserts to Vectorize with metadata `{source_url, project_slug, chunk_index, text}`. Verified end-to-end: 92 chunks across 13 projects ingested, retrieval scores 0.67-0.83 on representative queries.
